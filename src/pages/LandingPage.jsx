...
export const LandingPage = () => {
  ...
  // sliding cards data representing features — new palette (amber / rose / teal / indigo / sky)
  // replaces the old violet/fuchsia/cyan set so this section reads distinct from the rest of the page
  const featuresData = [
    {
      title: 'Generate Page',
  ...
  // Workflow timeline steps — brief > draft > personas > publish
  const workflowSteps = [
    {
      title: 'Brief',
      icon: FileText,
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-100 overflow-hidden font-display select-none">
      ...
      <div className="relative w-64 h-64 mx-auto my-10 z-10 flex items-center justify-center">
        ...
        {/* Code-drawn core — no external image, so it stays perfectly circular and always loads */}
        <motion.div
          className="relative w-56 h-56 rounded-full z-10 pointer-events-none"
          animate={{ y: [0, -15, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
        >
          {/* Sphere base: layered radial gradient gives it volume/depth */}
          <div
            className="absolute inset-0 rounded-full"
            style={{
              background:
                'radial-gradient(circle at 34% 28%, rgba(255,255,255,0.95) 0%, rgba(196,181,253,0.9) 10%, rgba(139,92,246,0.85) 28%, rgba(76,29,149,0.9) 52%, rgba(15,17,33,1) 78%)',
              boxShadow: '0 0 70px rgba(139,92,246,0.45), inset -18px -18px 50px rgba(0,0,0,0.55)'
            }}
          />

          {/* Latitude/longitude lines — reads as a globe/core rather than a flat circle */}
          ...
          {/* Slow rotating dashed ring for a live, running-system feel */}
          <motion.svg
            viewBox="0 0 200 200"
      ...
      <div className="max-w-6xl mx-auto px-6 pb-24 relative z-10 text-center">
        <div className="space-y-2 mb-10">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center justify-center gap-2">
            <Zap size={14} className="text-amber-400" /> Transform Your Workflow
          </h2>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-24 relative z-10">
        <div className="text-center space-y-2 mb-14">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center justify-center gap-2">
            <Workflow size={14} className="text-amber-400" /> How It Flows
          </h2>
