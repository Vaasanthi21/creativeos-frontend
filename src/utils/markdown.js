...
const parseMarkdownTables = (html) => {
  ...
  const generateHtmlTable = (header, alignments, rows) => {
    let tableHtml = '<table class="min-w-full divide-y divide-slate-200 dark:divide-white/5 text-sm my-6 border border-slate-200 dark:border-white/10 rounded-xl" style="width: 100%; border-collapse: collapse; margin: 24px 0; border: 1px solid #e2e8f0;">\n';
    
    // Header
    tableHtml += '  <thead class="bg-slate-50 dark:bg-white/5 text-slate-700 dark:text-slate-200" style="background-color: #f8fafc;">\n';
    tableHtml += '    <tr>\n';
    header.forEach((cell, idx) => {
      ...
      tableHtml += `      <th class="px-4 py-3 ${alignClass} font-semibold text-slate-700 dark:text-slate-200" style="${alignStyle} padding: 12px 16px; font-weight: 600; border: 1px solid #e2e8f0; color: #334155;">${cell}</th>\n`;
    });
    tableHtml += '    </tr>\n';
    tableHtml += '  </thead>\n';
    
    // Body
    tableHtml += '  <tbody class="divide-y divide-slate-100 dark:divide-white/5 text-slate-700 dark:text-slate-300">\n';
    rows.forEach(row => {
...
export const renderMarkdownToHTML = (markdown) => {
  ...
  html = html.replace(/```(?:[a-zA-Z0-9]+)?\r?\n([\s\S]*?)\r?\n```/g, (match, code) => {
    return `<pre class="bg-slate-900/90 dark:bg-black/40 border border-slate-200 dark:border-white/5 p-4 rounded-xl font-mono text-xs text-slate-200 dark:text-slate-300 my-4 overflow-x-auto select-text"><code>${code}</code></pre>`;
  });
  ...
  // 3. Headings: #, ##, ###, ####, #####, ######
  html = html.replace(/^###### (.*?)$/gm, '<h6 class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-3 mb-1">$1</h6>');
  html = html.replace(/^##### (.*?)$/gm, '<h6 class="text-xs font-bold text-slate-600 dark:text-slate-300 mt-4 mb-2">$1</h6>');
  html = html.replace(/^#### (.*?)$/gm, '<h5 class="text-xs md:text-sm font-bold text-slate-800 dark:text-white mt-4 mb-2">$1</h5>');
  html = html.replace(/^### (.*?)$/gm, '<h4 class="text-sm md:text-base font-bold text-slate-900 dark:text-white mt-5 mb-2">$1</h4>');
  html = html.replace(/^## (.*?)$/gm, '<h3 class="text-base md:text-lg font-bold text-slate-900 dark:text-white mt-6 mb-3">$1</h3>');
  html = html.replace(/^# (.*?)$/gm, '<h2 class="text-lg md:text-xl font-bold text-slate-900 dark:text-white mt-7 mb-4">$1</h2>');
  ...
  html = html.replace(/_([^_]+)_/g, '<em class="italic text-slate-600 dark:text-slate-400">$1</em>');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if ((line.startsWith('- ') || line.startsWith('* ')) && !inPre) {
      ...
      if (!inList) {
        lines[i] = `<ul class="list-disc ml-5 my-3 text-slate-700 dark:text-slate-300 space-y-1"><li>${content}</li>`;
        inList = true;
      } else {
      ...
      if (line && !isBlockHtml && !inPre) {
        lines[i] = `<p class="mb-3 leading-relaxed text-slate-700 dark:text-slate-300">${line}</p>`;
      }
    }
