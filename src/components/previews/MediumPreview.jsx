...
export const MediumPreview = ({ title, subtitle, copy, imageUrl, companyLogo }) => {
    ...
    <div className="glass-card rounded-3xl border border-border max-w-2xl mx-auto p-6 md:p-8 bg-card text-left shadow-2xl relative">
      ...
      <div className="flex items-center justify-between border-b border-border pb-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xs font-bold text-foreground hover:underline cursor-pointer">Veloce Operations</h4>
            <p className="text-[10px] text-muted-foreground mt-0.5">Sourced from DevOps Scalers • 6 min read • 🗓️ June 2026</p>
          </div>
        </div>
        
        <div className="flex items-center gap-2 text-muted-foreground">
          <button className="p-1.5 hover:bg-muted hover:text-foreground rounded-lg transition-colors"><Bookmark size={15} /></button>
          <button className="p-1.5 hover:bg-muted hover:text-foreground rounded-lg transition-colors"><Share2 size={15} /></button>
        </div>
      </div>
      ...
      <div className="space-y-5 leading-relaxed font-sans max-h-[380px] overflow-y-auto pr-2 scrollbar-glass">
        ...
        {subtitle && (
          <h4 className="text-xs md:text-sm font-normal text-muted-foreground leading-normal font-serif mt-1">
            {subtitle}
          </h4>
        )}
      ...
      <div className="pt-5 border-t border-border mt-6 flex items-center justify-between text-xs text-muted-foreground font-mono">
        <div className="flex items-center gap-4">
          <button 
            onClick={handleClap}
            className={`flex items-center gap-1.5 hover:text-foreground transition-all duration-200 relative ${
              animating ? 'scale-125' : ''
            }`}
          >
            ...
            <span className="font-semibold text-foreground">{claps} Claps</span>
            {animating && (
